...
extern double ** kernelM; 

double custom_kernel(KERNEL_PARM *kernel_parm, SVECTOR *a, SVECTOR *b) 
     /* plug in you favorite kernel */                          
{
	//printf("check %d %d = %lf \n",atoi(a->userdefined),atoi(b->userdefined),kernelM[atoi(a->userdefined)][atoi(b->userdefined)]);
	//return kernelM[atoi(a->userdefined)][atoi(b->userdefined)];
  	//return(sprod_ss(a,b));
	//printf("check %d %d = %lf \n", (int)(a->words[0].wnum) , (int)(b->words[0].wnum) , kernelM[(int)(a->words[0].wnum)][(int)(b->words[0].wnum)] );
	return kernelM[(int)(a->words[0].wnum)][(int)(b->words[0].wnum)]; 
  /* return((double)(1.0));  */
}
